...
async function seed() {
  ...
  // Cross-module models are imported at the top
  ...
  const engDept  = await Department.findOne({ name: /information technology/i });
  const engAssets = await Asset.find({ department: engDept?._id }).limit(5);

  if (!admin || !engDept) {
    console.error('❌ Required base data (admin user / Information Technology dept) not found. Run upstream seeds first.');
    process.exit(1);
  }
